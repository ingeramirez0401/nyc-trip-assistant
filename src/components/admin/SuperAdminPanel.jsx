...
const SuperAdminPanel = ({ onClose }) => {
  ...
  const fetchAgencies = useCallback((params) => adminService.listAgencies(params), []);
  const {
    items: agencies,
    ...
    setSearch,
    loading,
  } = usePaginatedList(fetchAgencies, { pageSize: 20 });
        ...
        <section className="bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-2xl p-6">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
            <h2 className="text-lg font-bold text-slate-900 dark:text-white">
              {t('agency:superAdmin.agenciesTitle', { count: total })}
            </h2>
            <div className="relative sm:w-72">
              ...
              <input
                ...
                placeholder={t('agency:superAdmin.searchPlaceholder')}
                className="w-full pl-9 pr-3 py-2.5 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-white/10 rounded-xl text-sm text-slate-900 dark:text-white focus:ring-2 focus:ring-indigo-500 outline-none"
              />
            </div>
          </div>
          ...
          {loading ? (
            ...
            <p className="text-slate-500 dark:text-slate-400 text-sm">
              {search ? t('agency:superAdmin.noResults') : t('agency:superAdmin.empty')}
            </p>
          ) : (
            <>
